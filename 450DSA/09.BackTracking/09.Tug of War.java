...
public class Main {
	...
	public static void solve(int[] arr, int vidx, ArrayList<Integer> set1, 
...
public class Main {
	...
	public static void solve(int[] arr, int vidx, ArrayList<Integer> set1, 
	                         ArrayList<Integer> set2, int soset1,int soset2)
	{
		if(vidx==arr.length && set1.size()+set2.size()==arr.length)
		{
		    if(Math.abs(soset1-soset2)<mindiff)
		    {   
		        ans=set1+" "+set2;
	   	...
	   	for(int i=vidx;i<arr.length;i++)
			...
			{	
			        set1.add(arr[i]);
			        solve(arr,i+1,set1,set2,soset1+arr[i],soset2);
			        set1.remove(set1.size()-1);
			}
			if(set2.size()<(arr.length+1)/2)
			{		
		        	set2.add(arr[i]);
		        	solve(arr,i+1,set1,set2,soset1,soset2+arr[i]);
		        	set2.remove(set2.size()-1);
			}
	       }
